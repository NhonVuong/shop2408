...
function loginFromSocialCallBack($socialUser) {
    ...
    $result = mysqli_query($con, "Select `id`,`username`,`fullname`,`email` from `users` WHERE `email` ='" . $socialUser['email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `users` (`fullname`,`email`, `updated_at`, `created_at`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', " . time() . ", '" . time() . "');");
        if (!$result) {
